...
def slope(dem, projection, transform):
    ...
    calculated using the slope_degrees method. 
    """
...
def fetch_dems(
    ...
    itemdir = f'{cell_id}_{state}_3DEP_{res}mDEM{suffix}'
    ...
    if (not os.path.exists(outfile)) or overwrite:
        ...
        width_buff = np.ceil((bbox_buff[2]-bbox_buff[0])/res).astype(int)
        height_buff = np.ceil((bbox_buff[-1]-bbox_buff[1])/res).astype(int)
        trf_buff = transform.from_bounds(*bbox_buff, width_buff, height_buff)
        ...
        try:
            bands = [
                # dem,
                slope(dem, p_crs, trf_buff),
                aspect(dem, p_crs, trf_buff),
                flow_accumulation(dem),
                tpi(dem, irad=15, orad=30, res=res),
                tpi(dem, irad=185, orad=200, res=res),
            ]
            bands.append(
                classify_slope_position(bands[4], bands[1])
            )
            bands.append(
                classify_landform(bands[4], bands[5], bands[1])
            )
        except Exception as e:
...
if __name__ == '__main__':
    ...
    out_path = DATADIR / 'interim' / '3dep-dtm'
